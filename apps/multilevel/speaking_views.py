    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    message_id = telegram_notifier.notify_new_submission(test_result, section='speaking')
                    if message_id:
                        manual_review.telegram_message_id = message_id
                        manual_review.save(update_fields=['telegram_message_id'])
                        logger.info(f"[SPEAKING] Saved telegram_message_id: {message_id}")
                except Exception as e:
                    logger.error(f"[SPEAKING] Failed to send Telegram notification: {e}")
